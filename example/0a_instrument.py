...
all_instruments(type=None, as_of=None, market=None, cached=True) -> DataFrame
  type：CS 股票、INDX 指数；省略则保留全部类型。
  as_of：身份快照日期；省略取最新。market：省略合并已绑定市场。
instruments(order_book_ids, as_of=None) -> Instrument / list[Instrument]
...
# [all_instruments.4]
# 3. 历史时点有哪些合约？as_of 决定身份快照，不表示当日一定有成交。
print(all_instruments(type="CS", as_of="2025-09-18", market="cn").head())
# [/all_instruments.4]

# [all_instruments.5]
# 4. 查询美股历史身份。美股类型使用 EQTY/FUND/DERV，不能用 CS 筛选。
# 当前客户端 type 参数只接受 CS/INDX，故先省略 type，再筛选返回表。
us_contracts = all_instruments(as_of="2025-06-16", market="us")
