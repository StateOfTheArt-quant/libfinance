...
上面说的都是区间的\ **右端**\ 。左端也可能有限制：服务端可能只允许查最近两年多的
历史，比这更早的 ``start_date`` 会被悄悄上拉到边界。

麻烦在于 ``end_date`` 如果也早于那个边界，它会\ **一起被上拉**——于是 start 和 end
撞在一起，返回一张空表。而空表看不出是"这段时间没有数据"还是"超出了可查范围"。

所以客户端在请求会被夹紧时会先给一句警告，把边界日期直接说出来。警告形如：

..  code-block:: text

    UserWarning: get_price: 可查区间的起点是 <边界日期>，比它更早的
                 start_date=<你传的> 会被服务端夹到边界（end_date 早于边界时也会
                 一起上拉，结果可能是空表）。

看到这句就把 ``start_date`` 调到边界之内。没有这句，说明你的区间没被夹。

..  note::

    不是每个部署都有这个限制。服务端没有声明区间限制时，这句警告不会出现，
    历史区间也不会被夹。
